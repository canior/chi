...
abstract class Alipay extends GatewayInterface
{
    ...
    public function init(array $config)
    {
        ...
        if (is_null($this->userConfig->get('app_id'))) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_ALI_PAY_CONFIG, ['app_id']);
        }
        // 沙箱模式
...
//            'return_url'  => $this->userConfig->get('return_url', ''),
    ...
    public function verify($data, $sign = null, $sync = false)
    {
        if (is_null($this->userConfig->get('public_key'))) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_ALI_PAY_CONFIG, ['public_key']);
        }
    ...
    protected function getResult($options, $method)
    {
        ...
        if (!isset($data[$method]['code']) || $data[$method]['code'] !== '10000') {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_PAY_HTTP_RESPONSE, [], "\nResultError" .
    ...
    protected function getSign()
    {
        if (is_null($this->userConfig->get('private_key'))) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_ALI_PAY_CONFIG, ['private_key']);
        }
