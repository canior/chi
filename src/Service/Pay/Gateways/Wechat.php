...
abstract class Wechat extends GatewayInterface
{
    ...
    public function init(array $config)
    {
        ...
        if (is_null($this->userConfig->get('app_id'))) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_WX_PAY_CONFIG, ['app_id']);
        }
        if (is_null($this->userConfig->get('mch_id'))) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_WX_PAY_CONFIG, ['mch_id']);
        }
        if (is_null($this->userConfig->get('mch_key'))) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_WX_PAY_CONFIG, ['mch_key']);
        }
...
//            $this->gateway_ras_public = 'https://fraud.mch.weixin.qq.com/sandboxnew/risk/getpublickey';
            ...
            if (empty($sandbox_signkey)) {
                ...
                if (isset($result['return_code']) && $result['return_code'] === 'SUCCESS') {
                    ...
                    CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_WX_PAY_CONFIG, [], isset($result['return_msg']) ? $result['return_msg'] : '沙箱验证签名及获取沙箱密钥失败！');
    ...
    protected function getResult($url, $cert = false)
    {
        ...
        if (isset($error)) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_PAY_HTTP_RESPONSE, [], $error);
        }
        return $data;
    }
    ...
    protected function toXml($data)
    {
        if (!is_array($data) || count($data) <= 0) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_PAY_COMMON, [], 'convert to xml error !invalid array!');
        }
        $xml = '<xml>';
    ...
    {
        if (!$xml) {
            CommonUtil::resultData()->throwErrorException(ErrorCode::ERROR_PAY_COMMON, [], 'convert to array error !invalid xml');
        }
